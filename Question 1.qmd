...
for(i in 1:ncol(results[[1]])){
  ...
  trace_plots[[i]] <- ggplot()+
    ...
    geom_text(aes(x = 0, y = 0.5, label = paste0("Mean=", round(mean(df$x), 1))), color = "red", size = 2) +
    geom_text(aes(x = 0, y = 0.3, label = paste0("True Beta=", round(beta.true[i], 1))),color = "green", size = 2) +
    labs(
